...
Use this when an external reviewer, CI, production signal, or user catches a live issue after a local review or architecture pass was recorded clean.

The goal is not to add a checklist for the exact bug. The goal is to identify the review failure mechanism and make the next review attack that mechanism before all-clear.
...
Do not add future review prompts for the exact escaped bug, file, framework, storage engine, entity name, or implementation tactic. Convert the issue into a reusable review mechanism at the highest useful level.

Use this abstraction ladder:

1. **Incident:** what failed in this repo today. Keep this in the review ledger.
...
4. **Proof pattern:** what future reviews must verify, independent of whether a repo uses filters, queries, joins, guards, policies, reducers, schemas, cache keys, route mappings, or another implementation shape.
...
- **Derived sink:** whether the admitted source set also feeds downstream ids, joins, aggregates, metadata fetches, counters, previews, permission summaries, side effects, or generated rows.
...
- **Prevention artifact:** regression test, static guard, invariant helper at the owner boundary, contract test for serialized output, generated freshness check, or review-ledger prompt.

## Root-Cause Patterns To Generalize

Use these patterns as review lenses. Do not mention the concrete escaped bug unless it is relevant to the current code.
...
Review each acquisition mode separately. An eligible parent, event, cache entry, generated row, or retained reference does not automatically make related data or downstream effects eligible.

### Derived ID Propagation

When an admitted or eligible collection is used to derive ids for another fetch, join, aggregate, metadata load, counter, preview, or generated row, prove those derived inputs came only from authorized sources and are validated again at the target owner when needed.
...
Before all-clear, test or inspect the behavior when the referenced object is missing, deleted, hidden, moved, or no longer readable.
...
## Review Procedure After An Escape

1. Import the finding normally.
2. Assign bug classes from `bug-class-taxonomy.md`.
3. Add the escape triage fields above to the review turn.
...
6. Before the next all-clear, add a short "escape prevention" note: what review question, test, guard, or architecture rule now prevents the class.

## All-Clear Addendum

If the branch has had an escaped live finding, do not give all-clear until:
...
- the review file records why the new proof is stronger than the prior clean review
