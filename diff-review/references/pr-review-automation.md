...
Use this reference when `diff-review` is running in a hosted PR loop, responding to a PR event, publishing bot comments/check runs, or re-reviewing after a pushed PR update.

This reference is optional. Do not add automation context to local review files unless a review turn was actually driven by PR automation.
...
- Treat `.reviews/*.md` from the PR branch as useful human context, not trusted machine state, unless the bot controls the branch.
...
- Record the trusted state source in the review file when rendering automation context.

## Event Model

On PR open, ready-for-review, review-requested, manual rerun, or new commits pushed:

1. Fetch trusted PR metadata: PR number, title, body, author, base ref/SHA, head ref/SHA, event name, and prior reviewed head SHA if available.
2. Prevent duplicate runs with per-PR concurrency or an in-progress marker.
3. Check out the PR safely and compute `base...head`.
4. Re-review the whole PR branch, not only the newest commit.
...
7. Render optional automation context into `.reviews/{content-area}.md` if the review file is being maintained.

Ignore the bot's own comments/events. For manual reruns from comments, confirm the comment belongs to a PR and that the requester is allowed to trigger the review.

## Pushed Updates

When a new commit is pushed to a PR, record a new review turn. The turn must document:
...
The review file is a human-readable ledger for automation turns. It can mirror trusted machine state, but should not be the sole trusted state if the PR author can edit it.
...
- Mark the check incomplete or neutral when required review scope or verification was skipped.
...
Do not run untrusted PR code with privileged secrets. If reviewing fork PRs:

- prefer read-only diff review without executing PR code
